...
/*Cria a div que permite a sele��o de todas as mensagens*/
...
function draw_box(headers_msgs, msg_folder, alternate){
	...
	numBox = 0; //As pastas sempre estar�o na aba 0
...
function make_tr_message(headers_msgs, msg_folder, offsetToGMT){
		...
		$(td_element26).click(function() {
            ...
            //Verificar se est� carregando a bandeira.
            //Caso esteja ele sai da fun��o at� que seja carregado.
            if( loading.length ) {
                return false;
            }
			...
			*/
			...
			if (!flagged) {
               ...
               var id = $(tr_element).addClass('selected_msg').find(':checkbox').attr('id');
                ...
                //Verifica se o checkbox est� selecionado
                if($('#' + id).attr('checked') != 'checked')
                    $(tr_element).addClass('selected_msg').find(':checkbox').trigger('click');

				/**
				 * Hack - For�a a atualiza��o da sele��o da mensagem, devido a problema na
				 * function de sele��o atribuida ao evento onclick do checkbox
				 */
...
function draw_message(info_msg, ID){
	...
	img_next_msg.style.cursor = 'pointer';
        ...
        //Corre��o para fazer funcionar e-mails assinados no formato encapsulado.
	...
	if (folder){ // mensagem local criptografada nao tem ID da pasta local
		...
		/*Ultima mensagem de cada p�gina, exceto a ultima*/
	...
	if (folder){ // mensagem local criptografada nao tem ID da pasta local
		...
		//primeira mensagem de cada p�gina, exceto a primeira
		else if(current_page > 1){
	...
	var div_other_options = document.createElement("DIV");
		...
		// Op��o do menu 'Mais A��es' para criar filtro a partir da mensagem aberta:
		...
		// CRIA��O DE OP��ES DE RESPONDER MENSAGEM
	...
	var update_labeleds_msg = function(){
		//TODO Mudar quando API abstrair atualiza��es no cache
		...
		$.ajax({
			...
			success: function(data){
				...
				case 4:
					...
					break;
				case 12:
					write_msg('Este evento n�o existe mais.');
					return;
					break;
				}
			}
		...
		try{
			if ( links.item( i ).href.indexOf( 'mailto:' ) === 0 ){
				...
				if ( ( links.item( i ).href.indexOf( 'javascript:' ) !== 0 ) &&
					(links.item( i ).href.indexOf(anchor_pattern) !== 0) ) //se n�o for �ncora
	...
	if ((info_msg.thumbs)&&(info_msg.thumbs.length > 0)){
		...
		ul_thumbs_list.setAttribute("className", "expressomail-thumbs-list"); //for IE
					...
					var i = 0;
				//verifica se est� no novo padr�o de montagem das mensagens ou no antigo, necess�rio
	...
	/*
	 * TODO: implementar o controle como prefer�ncia do usu�rio
	 *
	 */
	var jcarousel = false;
	if (jcarousel) {
					...
					'titleFormat' : function(name, currentArray, currentIndex, currentOpts) {
							...
							return '<div id="fancybox-title" class="fancybox-title-over" style="width: 100%; display: block;">' +
										'<div id="fancybox-title-over">' +
											...
											'<a title="Pr�xima" onclick="javascript:$.fancybox.next();" style="float:right;">' +
...
function valid_emails(email){
	...
	//FUN��O QUE VALIDA OS DADOS QUANDO O EMAIL � DIGITADO COM ("NOME SOBRENOME" <Email@dominio.com>)
	...
	switch (validation.length) {
		...
		case 1:
			...
			break;
		//CORRIGI ERRO DE DIGITA��O COMO ( huahua"<huhau@hauhau.com>) ou (hahahaha"huahua@email.com) ou ainda (hahahaha"huahua@ema  il.com)
		case 2:
			ContactBox = complexValidation(validation);
			break;
...
function input_keydowns(input, ID){
	...
	input.keydown(function(e){
		...
		setTimeout(function(){
            // CASO FOR PRESSIONADO "," OU ";", � CRIADA UMA CAIXINHA.
		...
		setTimeout(function(){
			...
			if(vchar == ">"){
				...
				for(i=0; i<input.val().length; i++){
					/*Se o caracter for mai�sculo, o valor de pixel � maior*/
			...
			/*Ao inserir novo contato, n�o existe a necessidade de calcular tamanho do campo*/
			else{
	...
	.focusout(function(e){
		var these = $(this);
		// Fun��o para montar a caixinha de e-mail.
			...
			if(box.find(".box-info").length){
				box.unbind("dblclick").bind("dblclick", function(e){
					new $.Zebra_Dialog('<strong>Impossivel editar</strong> um contato do ldap\n' +
						'<strong>Por�m</strong>� possivel remove-lo', {
						'buttons':  false,
						'modal': false,
						'position': ['right - 20', 'top + 20'],
						'auto_close': 3000,
...
//		'<span class="message_options_print"><span ' + change_font_color + ' title="'+get_lang("Print")+'" class="message_options" onclick=print_all()>'+get_lang("Print")+'</span></span>'+
    ...
    //Link arquivar e desarquivar com a��o
    //MAILARCHIVER
	...
	if (use_spam_filter) {
		...
		if (re.test(current_folder)) {
			btnSpam = '<span class="message_options_not_spam"><span ' + change_font_color + ' title="N�o Spam" class="message_options" onclick="nospam(\'selected\',\'null\',\'null\')">N�o Spam</span></span>';
		}
		else {
